...
public class Powerup extends StaticEntity {
    ...
    private static Animation powerup_animation;
    public Powerup(Handler handler, float x, float y) {
        ...
        powerup_animation = new Animation(180, Assets.powerup);
    }
    ...
    public boolean checkEntityCollisions(float xOffset, float yOffset) {
        ...
        if (check) {
            Sfx.PICKUP.play();
            handler.getWorld().getEntityManager().getPlayer().activateGodMode();
    ...
    private BufferedImage getCurrentAnimationFrame() {
       return powerup_animation.getCurrentFrame();
    }          
